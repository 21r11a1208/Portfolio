# Gymate Product Requirements Document (Unified)

- Canonical file: PRD.md
- Last updated: 2026-03-16
- Coverage: strategic requirements + execution requirements + delivery backlog

## Part I: Strategic Product Requirements
...
## Part II: Execution Requirements

## 1. Document Control

- Product: Gymate
- Version: 1.0 (Execution Edition)
- Date: 2026-03-16
- Status: Active
- Source context: `PRD.md`, `README.md`, `CODEBASE_INDEX.md`, current `mobile/` and `server/` implementation

## 2. Product Summary

Gymate is a mobile platform that helps users find compatible workout partners nearby using swipe-based discovery, match creation, and in-app chat. The MVP emphasizes speed to first match, safe interactions, and profile quality.

## 3. Problem Statement

Many gym-goers struggle with motivation, accountability, and confidence when training alone. Existing fitness apps focus on tracking workouts, not finding compatible training partners.

## 4. Goals and Non-Goals

### Goals (MVP)

- Enable sign-in and profile setup in under 5 minutes.
- Allow users to discover nearby candidates and perform swipe actions.
- Create matches and support real-time messaging between matched users.
- Provide baseline safety controls (block and report).
- Capture key analytics for moderation and operational visibility.

### Non-Goals (MVP)

- Subscription billing and premium entitlements.
- Group workouts/events.
- AI coaching or workout plan generation.
- Multi-language localization beyond default language.

## 5. Target Users

- Beginners who need accountability and confidence at the gym.
- Intermediate/advanced lifters seeking consistent partners or spotters.
- Social fitness users looking to expand workout circles.

## 6. Core User Journeys

### Journey A: Onboard -> Authenticate -> Discover

1. User launches app and completes onboarding slides.
2. User verifies phone with OTP.
3. User enters profile details.
4. User opens discovery and starts swiping.

Success criterion:
- User reaches first swipe within 5 minutes of first install.

### Journey B: Mutual Interest -> Match -> Chat

1. User likes another profile.
2. Mutual likes create a match.
3. User opens match thread and sends a first message.
4. Typing and read receipts update in chat.

Success criterion:
- User can send and receive messages in real time in a valid match.

### Journey C: Safety Action

1. User long-presses an incoming message and reports it.
2. User blocks a match partner.
3. Blocked chat is disabled for the relationship.

Success criterion:
- Block/report actions complete and prevent unsafe follow-up interaction.
...
- Persist token and user identity on device.
- Rehydrate session on app start.

Primary interfaces:
- Mobile: `mobile/src/screens/AuthScreen.js`, `mobile/src/store/slices/authSlice.js`
- API: `POST /api/auth/otp/request`, `POST /api/auth/otp/verify`

Acceptance criteria:
- Invalid OTP returns clear error and does not create session.
- Valid OTP returns token and user payload.
- App restores authenticated state after relaunch when token exists.

### 7.2 Profile Management

Requirements:
- Create and update user profile fields.
- Show completion progress for required profile fields.
- Sync device location to profile.

Primary interfaces:
- Mobile: `mobile/src/screens/ProfileScreen.js`, `mobile/src/components/ProfileForm.js`
- API: `GET /api/profiles/:id`, `POST /api/profiles`, `PUT /api/profiles/:id`, `POST /api/profiles/location/update`

Acceptance criteria:
- Required fields must be present before successful save.
- Location sync stores latitude/longitude and optional label.
- Saved profile is returned and reflected in app state.

### 7.3 Discovery and Swipe Actions

Requirements:
- Retrieve swipe queue with optional filters.
- Support pass, like, and super-like actions.
- Expose filter controls for radius, gender, and fitness level.

Primary interfaces:
- Mobile: `mobile/src/screens/DiscoverScreen.js`
- API: `GET /api/swipes/queue`, `POST /api/swipes/like`, `POST /api/swipes/pass`

Acceptance criteria:
- Queue returns candidates excluding self and invalid results.
- Swipe actions persist and advance to next card.
- Filters are applied to queue requests.

### 7.4 Matches and Messaging

Requirements:
- List user matches.
- Open a match thread and load messages.
- Send new messages.
- Broadcast and consume realtime events for message, read, and typing.
- Mark messages read.

Primary interfaces:
- Mobile: `mobile/src/screens/MatchesScreen.js`, `mobile/src/screens/ChatScreen.js`, `mobile/src/store/slices/chatSlice.js`
- API: `GET /api/matches`, `GET /api/chats/:matchId`, `POST /api/chats/:matchId`, `POST /api/chats/:matchId/read`, `POST /api/chats/:matchId/typing`
- Socket events: `chat:message`, `chat:read`, `chat:typing`

Acceptance criteria:
- Users can only access chat for authorized matches.
- New messages appear in sender and recipient threads without refresh.
- Read receipts update message status metadata.

### 7.5 Safety and Moderation

Requirements:
- Report a message from chat.
- Block and unblock users.
- Prevent chat access when relationship is blocked.
- Provide moderator queue and resolution endpoints.

Primary interfaces:
- Mobile: report and block actions in `mobile/src/screens/ChatScreen.js`
- API: `POST /api/chats/:matchId/report`, `GET/POST/DELETE /api/blocks`, `GET /api/moderation`, `POST /api/moderation/:id/resolve`

Acceptance criteria:
- Blocking returns success and disables chat input.
- Report endpoint stores moderation item.
- Moderator endpoints require moderator authorization.

## 8. Non-Functional Requirements

### Performance

- App cold start to interactive state under 3 seconds on mid-range devices.
- Swipe gesture feedback perceived as immediate (<300ms animation target).
- Chat send/receive under 1 second on stable networks.

### Reliability

- API endpoints return structured JSON errors.
- App handles API failures without crashes.
- Socket reconnect behavior should preserve thread continuity.

### Security

- JWT validation on protected endpoints.
- Authorization checks for profile, chat, and moderation actions.
- No sensitive secrets stored in source-controlled files.

### Privacy and Safety

- User location used only for matching features.
- Report/block data is auditable for moderation workflows.

## 9. Success Metrics (Execution KPIs)

- Activation: % users completing onboarding + OTP + profile setup.
- Discovery engagement: swipes per daily active user.
- Match conversion: matches per 100 likes.
- Messaging activation: % matches with first message in 24h.
- Safety rate: report/block events per 1,000 conversations.
...
- Stabilize auth, profile, discovery, match list, chat, and safety flows.
- Add critical integration tests for auth, swipes, and chat permissions.
- Validate production-ready environment configuration.

### Phase 2 (Post-MVP)

- Premium features and limits.
- Improved recommendation quality and richer filters.
- Push notifications for match/message events.
- Expanded moderation automation and trust signals.

## 11. Risks and Mitigations

- Legal risk around swipe interaction patterns.
  - Mitigation: maintain differentiated fitness-specific UX and legal review.
- Abuse and harassment in messaging.
  - Mitigation: fast block/report controls and moderator tooling.
- Cold start in low-density geographies.
  - Mitigation: radius fallback, profile quality nudges, and launch-city focus.
- Realtime instability on poor networks.
  - Mitigation: graceful retries, reconnect handling, and clear offline states.

## 12. Open Implementation Checks

- Ensure mobile match-fetch requests include required auth headers where endpoint enforcement requires it.
- Add explicit E2E checks for blocked-chat behavior and moderation item lifecycle.
- Confirm analytics event taxonomy for funnel and safety metrics before production rollout.

## 13. Traceability to Code
...
- Mobile socket client: `mobile/src/services/socket.js`
- Server app bootstrap: `server/src/app.js`, `server/src/index.js`
- Auth middleware: `server/src/middleware/auth.middleware.js`
- Controllers: `server/src/controllers/`
- Services: `server/src/services/`
- Data model: `server/prisma/schema.prisma`

## Part III: Delivery Backlog and Test Gates

## 15. Consolidated Implementation Backlog

### Epic 1: Authentication and Session

Story 1.1: OTP request/verify flow reliability
- Priority: P0
- Acceptance:
- Request endpoint enforces required fields and rate policy.
- Verify endpoint returns JWT + user identity.
- Client displays actionable errors for invalid/expired OTP.

Story 1.2: Session rehydration and sign-out integrity
- Priority: P0
- Acceptance:
- Relaunch restores session from persisted token/user.
- Sign-out clears local session artifacts.
- Protected views are inaccessible after sign-out.

### Epic 2: Profile Quality and Location

Story 2.1: Required profile completion enforcement
- Priority: P0
- Acceptance:
- Required fields blocked from save when missing.
- Completion meter updates as fields are filled.
- Save returns canonical profile from server.

Story 2.2: Location sync UX hardening
- Priority: P1
- Acceptance:
- Permission denied path handled with clear message.
- Sync success updates last-known location text.
- Invalid coordinates are rejected server-side.

### Epic 3: Discovery and Swipe

Story 3.1: Filtered queue consistency
- Priority: P0
- Acceptance:
- Queue request includes active filters.
- Empty queue and error states render correctly.
- Swipe card index remains consistent after refresh.

Story 3.2: Swipe action durability
- Priority: P0
- Acceptance:
- Pass/Like/Super-like are persisted via API.
- Failure path does not crash UI and allows retry.
- Match outcomes propagate to matches list.

### Epic 4: Matches and Real-Time Chat

Story 4.1: Authenticated matches fetch correctness
- Priority: P0
- Acceptance:
- Matches fetch uses authenticated context.
- Unauthorized response handled with recovery path.
- Match cards navigate to valid chat threads.

Story 4.2: Realtime message lifecycle
- Priority: P0
- Acceptance:
- Incoming socket message appends to active thread.
- Read receipts update status labels.
- Typing state appears/disappears with timeout behavior.

Story 4.3: Chat reliability under reconnect
- Priority: P1
- Acceptance:
- Socket reconnect restores room subscription.
- Duplicate events are not rendered twice.
- Pending state behavior is visible for transient failures.

### Epic 5: Trust and Safety

Story 5.1: Report pipeline completeness
- Priority: P0
- Acceptance:
- Long-press report submits messageId and context.
- Reported items appear in moderation queue.
- User receives success/failure feedback.

Story 5.2: Block enforcement end-to-end
- Priority: P0
- Acceptance:
- Block action disables chat input immediately.
- Blocked users cannot continue message exchange.
- Unblock endpoint restores eligibility where policy allows.

### Epic 6: Moderation and Analytics

Story 6.1: Moderator queue operations
- Priority: P1
- Acceptance:
- Moderator role required for queue access and resolution.
- Resolution action persists state transition.
- Audit trail fields are retained.

Story 6.2: KPI event taxonomy baseline
- Priority: P1
- Acceptance:
- Activation, match, chat, and safety events are defined.
- Event names/properties are consistent across services.
- Dashboard-ready aggregates can be produced from stored events.

## 16. Testing Strategy

### Minimum Automated Coverage (MVP)

- Auth integration tests: OTP request/verify + protected route access.
- Swipe/match integration tests: queue + like/pass + match creation behavior.
- Chat authorization tests: only matched users can access/send.
- Safety tests: report creation and block enforcement.

### Manual QA Checklist (Release Gate)

- New user can complete onboarding and first swipe.
- Mutual match can exchange messages in realtime.
- Blocked chat is unavailable and clearly communicated.
- Error states are understandable and non-fatal.
